...
body = (kicker('המדריך אצלכם') + h1('״להכין את הלב״ מחכה לכם כאן') +
    ...
    btn('לפתיחת המדריך', SITE+'assets/d/11103785b16fa692/lev-full.pdf') + divider() +
...
body = (kicker('יום 2') + h1('למה הבכור מתחיל פתאום להתנהג כמו תינוק') +
    ...
    para('ב״להכין את הלב״ יש על זה פרק שלם, עם דוגמאות ניסוח שמתאימות לגילאים שונים.') +
    divider() +
    para('ואם בא לכם להעמיק עכשיו, כתבתי על זה גם באתר: למה התגובות מגיעות דווקא אחרי הלידה ולא לפניה, מה שונה כשההפרש בין הילדים קטן, ואיך מגיבים לקנאה הראשונה.') +
    btn('לקרוא על אח חדש ויחסים בין האחים', SITE+'siblings.html') + sig())
...
body = (kicker('יום 5') + h1('למי ניגשים קודם כששניהם בוכים') +
    para('״הוא התחיל לצרוח, ותוך שנייה גם היא בכתה. עמדתי באמצע הסלון עם שתי ידיים, וחשבתי לעצמי שאני לא מספיקה לאף אחד מהם.״') +
    para('היי {$first_name|default(\'לכם\')}, את המשפט הזה שמעתי בשינויים קלים מעשרות הורים, ואמרתי אותו גם בעצמי יותר מפעם אחת.') +
    para('הדבר שלקח לי הכי הרבה זמן להבין הוא שגם הילד שלא מתפרץ נמצא בתוך הרגע. הוא שומע את הצעקות, רואה שתשומת הלב עברה, ומתחיל לבכות או להיצמד. לא כדי להקשות עליכם, אלא מפני שגם הוא זקוק לעזרה במה שקורה סביבו.') +
    kicker('מה שעוזר לי ברגע הזה') +
    ...
    divider() +
    para('כתבתי על זה בהרחבה בעמוד הפתוח על טנטרומים והתפרצויות: מה קורה לילד ברגע עצמו, למה ההסברים שלנו לא נקלטים כשהוא מוצף, ומה משתנה כשיש שני קטנים בבית.') +
    btn('לקרוא על הרגע עצמו', SITE+'tantrums.html') +
    para('ואם יש אצלכם רגע כזה שחוזר, השיבו למייל הזה וספרו לי עליו. אני קוראת הכל.') + sig())
EMAILS['drip-3'] = ('למי ניגשים קודם כששניהם בוכים', shell('למי ניגשים קודם כששניהם בוכים', body, 'המשפט שחוזר אצלי כמעט בכל שיחה עם הורים לצמודים'))
...
body = (kicker('יום 9') + h1('הרגע שבו יובל שאל אם תום חוזר לבית חולים') +
    ...
    divider() +
    # 25.9: ״לגדול ביחד״ ירד ממכירה; ההמשך מוביל ל״הורות בדאבל״ (הטקסט מעמוד התודה של שעת העומס)
    para('בדיוק בשביל הרגעים האלה יצרתי את ״הורות בדאבל״, כדי לעזור לכם להבין מה כל ילד צריך ולדעת מה לעשות גם כשהכול קורה יחד.') +
    btn('להכיר את ״הורות בדאבל״', DOUBLE) +
...
# ---------- C. מתנות חדשות (25.9) — הטקסט מעמודי התודה ומהשאלון של שקמה ----------
body = (kicker('הצ׳קליסט אצלך') + h1('מה עושים כששניהם צריכים אותי עכשיו?') +
    btn('להורדת הצ׳קליסט (PDF)', SITE+'assets/d/b36f3b5e2ea9d89a/checklist-shneihem.pdf') + divider() +
    kicker('ומה אם אותם רגעים חוזרים שוב ושוב?') +
    para('הצ׳קליסט יעזור לך לדעת מה לעשות ברגע עצמו. מדריך ״הורות בדאבל״ יעזור לך גם להבין מה עומד מאחורי ההתנהגות, מה אפשר להכין מראש ואיך להגיב בתוך גבולות, טנטרומים, קנאה ומריבות.') +
    btn('להכיר את ״הורות בדאבל״', DOUBLE) + sig())
EMAILS['magnet-checklist'] = ('מה עושים כששניהם צריכים אותי עכשיו?', shell('מה עושים כששניהם צריכים אותי עכשיו?', body, 'הצ׳קליסט אצלך'))

body = (kicker('המיני־מדריך אצלך') + h1('שעת העומס בדאבל') +
    btn('להורדת המיני־מדריך (PDF)', SITE+'assets/d/c7f508ddfe55c6b2/shaat-haomes.pdf') + divider() +
    kicker('ומה קורה כשהכול משתנה ברגע?') +
    para('עכשיו יש לך דרך לשנות שעה אחת שחוזרת אצלכם ולהכין מראש את מה שאפשר. אבל יהיו גם רגעים שלא יכולת להכין: אחד הילדים מתפרק בדיוק כשהשני צריך אותך, או שהצרכים משתנים באמצע.') +
    para('בדיוק בשביל הרגעים האלה יצרתי את ״הורות בדאבל״, כדי לעזור לך להבין מה כל ילד צריך ולדעת מה לעשות גם כשהכול קורה יחד.') +
    btn('להכיר את ״הורות בדאבל״', DOUBLE) + sig())
EMAILS['magnet-rush'] = ('שעת העומס בדאבל', shell('שעת העומס בדאבל', body, 'המיני־מדריך אצלך'))

body = (kicker('השאלון') + h1('איפה הדאבל פוגש אותך?') +
    para('זה בדיוק הרעיון של ״הורות בדאבל״: לא לנסות לפתור כל בכי, התנגדות או מריבה בנפרד, אלא להבין מה כל ילד צריך ולדעת מה לעשות גם כשהכול קורה יחד.') +
    para('במדריך תקבלי דרך ברורה שתוכלי לחזור אליה בכל פעם ששניהם צריכים אותך ואת לא בטוחה מה לעשות קודם. כדי שתוכלי לפעול עם יותר בהירות ופחות צעקות, ניחושים ואשמה.') +
    btn('להכיר את ״הורות בדאבל״', DOUBLE) + sig())
...
def funnel():
    ...
    body = (kicker('המתנה אצלכם') + h1("{$gift_name|default('המתנה שלכם')}") +
        polaroid("{$gift_cover|default('https://imale.co/assets/photos/pola-home-420.jpg')}", 'כריכת המתנה', 150) +
        para(HI + "<br>{$gift_desc|default('הנה המתנה שביקשתם מהאתר.')}") +
        ...
        REPLY + sig())
    out['funnel-1'] = ("הנה מה שביקשתם: {$gift_name|default('המתנה שלכם')}", shell('המתנה אצלכם', body, 'הקישור בפנים, ומי אני בכלל'))

    # מייל 2: הרגע ששניהם בוכים (מתוך מייל 3 של ״להכין את הלב״) + המתנה הבאה + הקהילה
    body = (kicker('יום 2') + h1('למי ניגשים קודם כששניהם בוכים') +
        para('״הוא התחיל לצרוח, ותוך שנייה גם היא בכתה. עמדתי באמצע הסלון עם שתי ידיים, וחשבתי לעצמי שאני לא מספיקה לאף אחד מהם.״') +
        para(HI + ' את המשפט הזה שמעתי בשינויים קלים מעשרות הורים, ואמרתי אותו גם בעצמי יותר מפעם אחת.') +
        para('הדבר שלקח לי הכי הרבה זמן להבין הוא שגם הילד שלא מתפרץ נמצא בתוך הרגע. הוא שומע את הצעקות, רואה שתשומת הלב עברה, ומתחיל לבכות או להיצמד. לא כדי להקשות עליכם, אלא מפני שגם הוא זקוק לעזרה במה שקורה סביבו.') +
        kicker('מה שעוזר לי ברגע הזה') +
        ul(['אין כלל שאומר תמיד לצעיר או תמיד לבכור. בודקים מי זקוק לעזרה דחופה יותר ומי יכול להחזיק עוד רגע.',
            'לילד שמחכה מגיעה אמירה קצרה: ״אני רואה אותך. אני מסיימת עם אחיך ומגיעה אלייך.״ משפט אחד וקשר עין מחזיקים יותר ממה שנדמה.',
            'כשהסערה נגמרת, שווה לחזור לילד שחיכה ולתת לו כמה דקות שקטות שהן רק שלו.']) +
        divider() + para('חשבתי שגם זה יכול לעזור לכם:') +
    ...
    out['funnel-2'] = ('למי ניגשים קודם כששניהם בוכים', shell('למי ניגשים קודם כששניהם בוכים', body, 'המשפט שחוזר אצלי כמעט בכל שיחה עם הורים לצמודים'))

    # מייל 3: הסיפור של יובל ותום (מתוך מייל 4 של ״להכין את הלב״) ו״הורות בדאבל״
    body = (kicker('יום 5') + h1('הרגע שבו יובל שאל אם תום חוזר לבית חולים') +
        polaroid(SITE + 'assets/photos/shk-bench-420.jpg', 'שקמה על ספסל בפארק עם שני הילדים', 190) +
        para(HI + '<br>כמה שבועות אחרי שתום נולד, יובל שאל אותי בקול הכי רגיל בעולם אם התינוק חוזר עכשיו לבית חולים. בלי כעס. בסקרנות. ובאותו רגע הבנתי שכל ההכנה שעשינו לפני הלידה הייתה רק ההתחלה, והעבודה האמיתית מתחילה בשגרה: מי מקבל אותי כשאני חוזרת הביתה, מי יושב עליי בסיפור, מה קורה כששניהם בוכים ואני לבד.') +
        para('בדיוק בשביל הרגעים האלה יצרתי את ״הורות בדאבל״, כדי לעזור לכם להבין מה כל ילד צריך ולדעת מה לעשות גם כשהכול קורה יחד.') +
        ...
        btn('להכיר את ״הורות בדאבל״', DOUBLE) +
        small('מדריך דיגיטלי ממוקד ומעשי לחיים עם שני קטנטנים, בפרקים קצרים שאפשר לעבור ישר לנושא שהכי מעסיק אתכם.') +
        REPLY + sig())
    out['funnel-3'] = ('הרגע שבו יובל שאל אם תום חוזר לבית חולים', shell('הרגע שבו יובל שאל אם תום חוזר לבית חולים', body, 'סיפור מהבית שלי, ומה נולד ממנו'))
    return out
